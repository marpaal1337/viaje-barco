...
Modo manual (genera URLs de Google Flights para rellenar):
...
OUTPUT_FILE = os.path.join(DATA_DIR, "vuelos.json")
...
AIRLINE_MAP = {"FR": "Ryanair", "IB": "Iberia", "I2": "Iberia Express", "VY": "Vueling"}


def build_id(origin, dest, airline, date_str, dep_time):
...
def normalize_flights(flights, origin_name, dest_name, tipo):
    for f in flights:
        f["id"] = build_id(f["origen"]["codigo"], f["destino"]["codigo"],
    ...
    with sync_playwright() as p:
        ...
        for origin, dest, origin_name, dest_name in ROUTES:
            for date_str in DATES:
                ...
                print(f"[{count}/{total}] {origin}->{dest} {date_str} ({tipo})")
...
def _scrape_ryanair(page, origin, dest, date_str):
    url = (
        ...
        f"?adults=1&dateOut={date_str}&originIata={origin}&destinationIata={dest}"
    )
...
def generate_urls():
    ...
    for origin, dest, origin_name, dest_name in ROUTES:
        for date_str in DATES:
            ...
            urls.append((origin, dest, date_str, tipo, gf_url))
            print(f"{origin}->{dest} {date_str} ({tipo})")
            print(f"  {gf_url}")
            print()

    print(f"Total: {len(urls)} rutas")
    return urls
...
def edit_prompt():
    ...
    flights = data.get("alternativas", [])
    print(f"\nEditando {len(flights)} vuelos. Deja vacío para mantener precio actual.\n")

    for i, f in enumerate(flights):
        ruta = f"{f['origen']['codigo']}->{f['destino']['codigo']}"
        print(f"[{i+1}/{len(flights)}] {f['aerolinea']} {ruta} {f['fecha']} {f['salida']}-{f['llegada']}")
        current = f["precio_eur"]
        inp = input(f"  Precio actual: {current} EUR -> Nuevo precio (EUR): ").strip()
        if inp:
            try:
                f["precio_eur"] = float(inp.replace(",", "."))
                print(f"  OK Actualizado a {f['precio_eur']} EUR")
            except ValueError:
                print(f"  - Mantenido: {current} EUR")

    # Recalcular combinaciones
    data["combinaciones"] = build_combinations(flights)
    ...
    print(f"\nGuardado: {OUTPUT_FILE} ({len(flights)} vuelos)")
...
def main():
    print("=" * 60)
    print("  Scraper de Vuelos - Viaje Baleares")
    ...
    if "--auto" in sys.argv:
        ...
        data = {
            "metadata": {
                ...
                "fuente": "scraper_vuelos.py (Playwright)",
            },
        ...
        print(f"\n  {len(flights)} vuelos guardados en {OUTPUT_FILE}")
        ...
        print("""
  Uso:
    python scraper_vuelos.py --auto      # Scraping automático (Playwright)
    python scraper_vuelos.py --urls      # Genera URLs de Google Flights
    python scraper_vuelos.py --editar    # Edita precios manualmente

  Requisitos:
    - Playwright: pip install playwright && playwright install chromium
    - O usa --urls y --editar para modo manual
""")
